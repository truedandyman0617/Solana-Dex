...
/// Creates and invokes a [swap_client::instruction::initialize] instruction.
///
/// # Arguments:
///
...
pub fn initialize<'a, 'b, 'c, 'info>(
    ...
    nonce: u8,
    amp_factor: u64,
    fees: swap_client::fees::Fees,
) -> ProgramResult {
    let ix = swap_client::instruction::initialize(
...
/// Creates and invokes a [swap_client::instruction::deposit] instruction.
///
/// # Arguments:
///
...
pub fn deposit<'a, 'b, 'c, 'info>(
    ...
    let ix = swap_client::instruction::deposit(
        // token program ID is verified by the stable swap program
...
/// Creates and invokes a [swap_client::instruction::swap] instruction.
///
/// # Arguments:
///
...
pub fn swap<'a, 'b, 'c, 'info>(
    ...
    let ix = swap_client::instruction::swap(
        ctx.accounts.user.token_program.key,
        ctx.accounts.user.swap.key,
        ctx.accounts.user.swap_authority.key,
        ctx.accounts.user.user_authority.key,
...
/// Creates and invokes a [swap_client::instruction::withdraw_one] instruction.
///
/// # Arguments:
///
...
pub fn withdraw_one<'a, 'b, 'c, 'info>(
    ...
    let ix = swap_client::instruction::withdraw_one(
        ctx.accounts.user.token_program.key,
        ctx.accounts.user.swap.key,
        ctx.accounts.user.swap_authority.key,
        ctx.accounts.user.user_authority.key,
...
/// Creates and invokes a [swap_client::instruction::withdraw] instruction.
///
/// # Arguments:
///
/// * `pool_token_amount` - Amount of LP tokens to withdraw.
...
pub fn withdraw<'a, 'b, 'c, 'info>(
    ...
    let ix = swap_client::instruction::withdraw(
        // token program ID is verified by the stable swap program
        ctx.accounts.user.token_program.key,
        ctx.accounts.user.swap.key,
        ctx.accounts.user.swap_authority.key,
...
/// Creates and invokes a [swap_client::instruction::ramp_a] instruction.
///
/// # Arguments:
///
...
pub fn ramp_a<'a, 'b, 'c, 'info>(
    ...
    let ix = swap_client::instruction::ramp_a(
...
/// Creates and invokes a [swap_client::instruction::stop_ramp_a] instruction.
pub fn stop_ramp_a<'a, 'b, 'c, 'info>(
    ctx: CpiContext<'a, 'b, 'c, 'info, AdminUserContextWithClock<'info>>,
) -> ProgramResult {
    let ix = swap_client::instruction::stop_ramp_a(
        ctx.accounts.admin_ctx.swap.key,
        ctx.accounts.admin_ctx.admin.key,
    )?;
    solana_program::program::invoke_signed(&ix, &ctx.to_account_infos(), ctx.signer_seeds)
...
/// Creates and invokes a [swap_client::instruction::pause] instruction.
pub fn pause<'a, 'b, 'c, 'info>(
    ctx: CpiContext<'a, 'b, 'c, 'info, AdminUserContext<'info>>,
) -> ProgramResult {
    let ix = swap_client::instruction::pause(ctx.accounts.swap.key, ctx.accounts.admin.key)?;
    solana_program::program::invoke_signed(&ix, &ctx.to_account_infos(), ctx.signer_seeds)
}

/// Creates and invokes a [swap_client::instruction::unpause] instruction.
pub fn unpause<'a, 'b, 'c, 'info>(
    ctx: CpiContext<'a, 'b, 'c, 'info, AdminUserContext<'info>>,
) -> ProgramResult {
    let ix =
        swap_client::instruction::unpause(ctx.accounts.swap.key, ctx.accounts.admin.key)?;
    solana_program::program::invoke_signed(&ix, &ctx.to_account_infos(), ctx.signer_seeds)
}

/// Creates and invokes a [swap_client::instruction::apply_new_admin] instruction.
pub fn apply_new_admin<'a, 'b, 'c, 'info>(
    ctx: CpiContext<'a, 'b, 'c, 'info, AdminUserContextWithClock<'info>>,
) -> ProgramResult {
    let ix = swap_client::instruction::apply_new_admin(
        ctx.accounts.admin_ctx.swap.key,
        ctx.accounts.admin_ctx.admin.key,
    )?;
    solana_program::program::invoke_signed(&ix, &ctx.to_account_infos(), ctx.signer_seeds)
...
/// Creates and invokes a [swap_client::instruction::commit_new_admin] instruction.
/// # Arguments:
///
...
pub fn commit_new_admin<'a, 'b, 'c, 'info>(
    ...
    let ix = swap_client::instruction::commit_new_admin(
        admin_ctx.swap.key,
        admin_ctx.admin.key,
...
/// Creates and invokes a [swap_client::instruction::set_fee_account] instruction.
pub fn set_fee_account<'a, 'b, 'c, 'info>(
    ctx: CpiContext<'a, 'b, 'c, 'info, SetFeeAccount<'info>>,
) -> ProgramResult {
    let ix = swap_client::instruction::set_fee_account(
        ctx.accounts.admin_ctx.swap.key,
        ctx.accounts.admin_ctx.admin.key,
...
/// Creates and invokes a [swap_client::instruction::set_new_fees] instruction.
/// # Arguments:
///
/// * `fees` - new fees
pub fn set_new_fees<'a, 'b, 'c, 'info>(
    ...
    let ix = swap_client::instruction::set_new_fees(
        ctx.accounts.swap.key,
        ctx.accounts.admin.key,
        fees,
    )?;
...
pub struct SwapInfo(swap_client::state::SwapInfo);

impl SwapInfo {
    /// The length, in bytes, of the packed representation
    pub const LEN: usize = swap_client::state::SwapInfo::LEN;
...
impl Deref for SwapInfo {
    type Target = swap_client::state::SwapInfo;
...
impl anchor_lang::AccountDeserialize for SwapInfo {
    ...
    fn try_deserialize_unchecked(buf: &mut &[u8]) -> Result<Self, ProgramError> {
        swap_client::state::SwapInfo::unpack(buf).map(SwapInfo)
    }
}
